...
public class MainWindow extends JFrame {
	...
	public MainWindow() {
		...
		btnConvert.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				String codeCurrency1 = currencies.get(comboBoxCountry1.getSelectedIndex()).getShortName();
				String codeCurrency2 = currencies.get(comboBoxCountry2.getSelectedIndex()).getShortName();
				String result;
				...
				price = convert(codeCurrency1, codeCurrency2, amount);
		
				// Format numbers to avoid "E7" problem
				formattedPrice = format.format(price);
				formattedAmount = format.format(amount);
		
				result = formattedAmount + " " + codeCurrency1 + " = " + formattedPrice + " " + codeCurrency2;
    ...
    // Convert method using RateManager
    public static Double convert(String currencyCode1, String currencyCode2, Double amount) {
        RateManager rateManager = RateManager.getInstance();
        Double exchangeRate = rateManager.getExchangeRate(currencyCode1, currencyCode2);
